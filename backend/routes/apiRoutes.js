...
router.get("/jobs", jwtAuth, (req, res) => {
  ...
  if (req.query.jobType) {
    ...
    span.addEvent("Job types filter applied", { jobTypes });
    findParams = {
      ...findParams,
      jobType: {
        $in: jobTypes,
  ...
  span.addEvent("Find and sort parameters set", { findParams, sortParams });
  ...
  span.addEvent("Aggregation pipeline set", { arr });
  ...
  Application.findOne({
    ...
    .then((appliedApplication) => {
      if (appliedApplication !== null) {
        span.addEvent("User has already applied for this job");
        res.status(400).json({
                    ...
                    if (myActiveApplicationCount < 10) {
                      Application.countDocuments({
                        ...
                        if (acceptedJobs === 0) {
                          ...
                          span.addEvent("User already has an accepted job");
                          res.status(400).json({
                            message:
                      ...
                      span.addEvent("User has 10 active applications");
                      res.status(400).json({
                        message:
