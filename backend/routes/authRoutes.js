...
router.post("/signup", (req, res) => {
  diag.info('Received signup request');
  const data = req.body;
  let user = new User({
    email: data.email,
      ...
      userDetails
        ...
        .catch((err) => {
          ...
          user
            .delete()
            .then(() => {
              diag.info('User deleted due to error in saving details', { userId: user._id });
              res.status(400).json(err);
            })
            .catch((err) => {
              diag.error('Error deleting user', { error: err });
              res.json({ error: err });
            });
          err;
        });
...
router.post("/login", (req, res, next) => {
  diag.info('Received login request');
