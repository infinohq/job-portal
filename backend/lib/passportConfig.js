  ...
  new Strategy(
    ...
    (req, email, password, done, res) => {
      ...
      User.findOne({ email: email }, (err, user) => {
        ...
        if (!user) {
          diag.warn('User does not exist for email:', email);
        ...
        user
          ...
          .catch((err) => {
            diag.error('Password is incorrect for user:', user._id);
...
passport.use(
  new JWTStrategy(
    ...
    (jwt_payload, done) => {
      diag.debug('Authenticating user with JWT payload:', jwt_payload);
      User.findById(jwt_payload._id)
        .then((user) => {
          diag.debug('User found with JWT payload:', jwt_payload._id);
          if (!user) {
            diag.warn('JWT Token does not exist for payload:', jwt_payload._id);
            return done(null, false, {
        ...
        .catch((err) => {
          diag.error('Incorrect Token for payload:', jwt_payload._id);
          return done(err, false, {
