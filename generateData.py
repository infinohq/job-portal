...
def generate_data(prompt):
    with tracer.start_as_current_span("generate_data"):
        trace.get_current_span().add_event(f"Generating data with prompt: {prompt}")
        response = openai.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[{"role": "user", "content": prompt}]
        )
        generated_text = response.choices[0].message.content.strip()
        trace.get_current_span().add_event(f"Generated text: {generated_text}")
...
def sign_up_recruiter(email, password, name, contact_number, bio):
    with tracer.start_as_current_span("sign_up_recruiter"):
        ...
        trace.get_current_span().add_event(f"Signing up recruiter with payload: {payload}")
        response = requests.post(f'{BASE_URL}/auth/signup', json=payload)
        trace.get_current_span().add_event(f"Response from recruiter signup: {response.status_code} - {response.text}")
...
def post_job(recruiter_token, title, skillsets, salary):
    with tracer.start_as_current_span("post_job"):
        ...
        trace.get_current_span().add_event(f"Posting job with payload: {payload}")
        response = requests.post(f'{BASE_URL}/api/jobs', headers=headers, json=payload)
        trace.get_current_span().add_event(f"Response from job posting: {response.status_code} - {response.text}")
        return response.status_code, response.json()
...
def sign_up_applicant(email, password, name, contact_number):
    with tracer.start_as_current_span("sign_up_applicant"):
        ...
        trace.get_current_span().add_event(f"Signing up applicant with payload: {payload}")
        response = requests.post(f'{BASE_URL}/auth/signup', json=payload)
        trace.get_current_span().add_event(f"Response from applicant signup: {response.status_code} - {response.text}")
        return response.status_code, response.json()
...
def clear_database():
    with tracer.start_as_current_span("clear_database"):
        ...
        trace.get_current_span().add_event(f"Response from clear database: {response.status_code}")
...
def create_synthetic_data(num_recruiters, num_jobs, num_applicants, clear_db=False):
    with tracer.start_as_current_span("create_synthetic_data"):
        if clear_db:
            trace.get_current_span().add_event("Clearing database...")
            clear_database()
        ...
        for i in range(num_recruiters):
            ...
            if status_code == 200:
                ...
                trace.get_current_span().add_event(f"Failed to sign up recruiter {name}: {response}")
        ...
        for token in recruiter_tokens:
            for _ in range(num_jobs // num_recruiters):
                ...
                if status_code != 200:
                    trace.get_current_span().add_event(f"Failed to post job: {response}")
        ...
        for i in range(num_applicants):
            ...
            if status_code != 200:
                trace.get_current_span().add_event(f"Failed to sign up applicant {name}: {response}")
